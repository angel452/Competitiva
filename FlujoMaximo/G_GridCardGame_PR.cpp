...
int main(void)
{
    ...
    for(int i = 1; i <= h; i++)
    {
        for(int j = 1; j <= w; j++){
            if(a[i][j] >= 0){
                //cout << "Mayor a 0" << endl;
                g.addEdge(i, h+j, a[i][j]);
                ans += a[i][j];
            }
            else{ 
                //cout << "Menor a 0" << endl;
                g.addEdge(h+j, i, 99999);
                rsum[i] -= a[i][j];
                csum[j] -= a[i][j];
            }
        }
    ...
    for(int i = 1; i <= h; i++){ 
        g.addEdge(S, i, rsum[i]);
    }

    for(int j = 1; j <= w; j++){
        g.addEdge(h+j, T, csum[j]);
    }

	cout << endl << "Utilizando Algoritmo Push-Relavel: " << endl;
    cout << ans - g.getMaxFlow(S, T) << endl;
    return 0;
}